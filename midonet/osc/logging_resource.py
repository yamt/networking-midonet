...
class DeleteLoggingResource(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for resource in parsed_args.logging_resource:
            try:
                obj = neutronV20.find_resourceid_by_name_or_id(
                    client, 'logging_resource', resource)
                client.delete_logging_resource(obj)
            except Exception as e:
                result += 1
                LOG.error(_("Failed to delete logging resource with "
...
class SetLoggingResource(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        client.update_logging_resource(obj, {'logging_resource': attrs})
...
class ShowLoggingResource(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        obj = neutronV20.find_resourceid_by_name_or_id(
            client, 'logging_resource',
            parsed_args.logging_resource)
        obj = client.show_logging_resource(obj)['logging_resource']
        columns = _get_columns(obj)
        data = utils.get_dict_properties(obj, columns, formatters=_formatters)
        return (columns, data)
